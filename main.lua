-- ПОЛНОСТЬЮ КАСТОМНЫЙ MM2 СКРИПТ (LEMONHUB СТИЛЬ)
...
-- Создаем основу для UI в PlayerGui (чтобы точно работало на любом инжекторе)
local MainGui = Instance.new("ScreenGui")
MainGui.Name = "LemonHubMM2"
MainGui.Parent = LocalPlayer:WaitForChild("PlayerGui")
MainGui.ResetOnSpawn = false

-- Функция создания закруглений
...
---------------------------------------------------------
-- ЧАСТЬ 1: ОСНОВНОЕ МЕНЮ (СПРЯТАНО ДО ВВОДА КЛЮЧА)
---------------------------------------------------------
...
-- Скрипт перетаскивания меню мышкой/пальцем
...
-- Маленькая плашка сверху экрана для разворачивания
...
-- Панель Вкладок (Сверху, как на скрине)
...
-- Функция добавления ползунка (Slider)
local function addSlider(parent, text, min, max, default, callback)
    ...
    -- Фикс сброса скольжения
    inputEndedReg = UIS.InputEnded:Connect(function(input)
...
---------------------------------------------------------
-- НАПОЛНЕНИЕ НАСТРОЕК (ФУНКЦИОНАЛ ЧИТА)
---------------------------------------------------------
...
-- Постоянное удержание скорости при ресете
task.spawn(function()
    while true do
        task.wait(0.5)
        pcall(function()
...
addSlider(PlayerPage, "Fly Speed (Скорость полета)", 16, 150, 16, function(v)
    flySpeed = v
end)

-- ВКЛАДКА VISUAL
addToggle(VisualPage, "ESP Роли (Шериф / Мардер)", function(v)
    _G.ESP = v
    while _G.ESP do
        task.wait(1)
...
-- ВКЛАДКА COMBAT (Пистолет и фарм)
...
KeyTitle.Text = "LEMONHUB | Ключ защиты"
...
Submit.MouseButton1Click:Connect(function()
    if InputBox.Text == "ilovepigs" then
        KeyFrame:Destroy() -- Удаляем окно ключа
        MenuFrame.Visible = true -- Показываем красивое меню
    else
        InputBox.Text = ""
